...
class FaceDetection(BaseModel):
    ...
    keypoints: Optional[List[Tuple[int, int]]] = Field(
        default=None, description="Optional facial keypoints"
    )
...
class FaceDetector:
    ...
    def __init__(self, min_detection_confidence: float = 0.7, model_selection: int = 0):
        """
    ...
    def get_face_crops(
        self, image: np.ndarray, detections: List[FaceDetection], padding: float = 0.2
    ...
    def filter_faces_by_confidence(
        self, faces: List[FaceDetection], threshold: float
    ) -> List[FaceDetection]:
        """
